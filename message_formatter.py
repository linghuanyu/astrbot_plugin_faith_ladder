...
def format_help(config: dict) -> str:
    ...
    return (
        ...
        f"\n"
        f"[玩家管理] (诸神权限)\n"
        f"{cmd_register} <姓名> <信仰> <职业> [登神之路分] [觐见分] - 录入新玩家（需目标发送祷词确认，会回显目标名片）\n"
        f"取消录入 [玩家名] - 取消进行中的录入（无参数取消自己发起的全部）\n"
        ...
        f"\n"
        f"[赠送道具]\n"
        f"赠送道具 <接收方名> <道具*数量> - 赠送（发送方自动识别，直接扣除，等待接收）\n"
        f"接受道具 - 接受赠送（诸神权限）\n"
        f"拒绝道具 - 拒绝赠送（诸神权限）\n"
        f"\n"
        f"[状态] (诸神权限)\n"
