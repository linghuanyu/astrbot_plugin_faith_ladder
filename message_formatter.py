...
def format_help(config: dict) -> str:
    ...
    return (
        ...
        f"清除状态 <玩家名> - 清除所有状态\n"
        f"重命名状态 <玩家名> <旧状态名> <新状态名> - 改名（目标名已存在则合并，到期取更晚的）\n"
        f"\n"
        f"[祈愿试炼]（需先绑定QQ；需在本群开启 wish_groups）\n"
        f"祈愿 - 本群招募中的队伍与当天的名额\n"
        f"祈愿组队 [人数] - 开一支队伍并自动入座（默认 {cfg_get(config, 'wish_default_capacity')} 人，最多 {cfg_get(config, 'wish_max_capacity')} 人；队名由系统生成）\n"
        f"祈愿加入 [队名] - 加入队伍（省略队名则进最缺人的一支）\n"
        f"祈愿退出 - 退出自己的队伍（队长退出即解散）\n"
        f"祈愿我的 - 查看我的队伍与成员\n"
        ...
        f"\n"
        f"[沉浸感] (诸神权限)\n"
        f"赌局 - 手动抛下一场「神明的赌局」（需开启 wager_enabled）\n"
        f"\n"
        f"[群管] (诸神权限)\n"
        f"禁言 <秒数> @用户 - 禁言（默认60秒）\n"
        f"解禁 @用户 - 解除禁言\n"
