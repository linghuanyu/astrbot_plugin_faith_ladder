...
class TestFormatPrayerTrigger:
    ...
    def test_positive_delta(self):
        ...
        msg = format_prayer_trigger("Alice", "虚无", 2, None)
    ...
    def test_negative_delta(self):
        from astrbot_plugin_faith_ladder.message_formatter import format_prayer_trigger
        msg = format_prayer_trigger("Alice", "存在", -1, None)
        assert "神明看到了你的祈祷" in msg
        assert "存在" in msg
        assert "-1" in msg
    ...
    def test_zero_delta(self):
        from astrbot_plugin_faith_ladder.message_formatter import format_prayer_trigger
        msg = format_prayer_trigger("Alice", "文明", 0, None)
        assert "神明看到了你的祈祷" in msg
        assert "文明" in msg
        assert "未起波澜" in msg
    ...
    def test_custom_config_messages(self):
        ...
        msg = format_prayer_trigger("Alice", "虚无", 1, config)
        assert "神明看到了你的祈祷" in msg
        assert "虚无开心，+1" in msg  # delta is formatted as "+1"
